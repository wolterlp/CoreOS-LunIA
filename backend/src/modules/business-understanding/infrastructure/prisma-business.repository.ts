...
export class PrismaBusinessRepository implements BusinessRepository {
  async saveProfile(profile: BusinessProfile): Promise<BusinessProfile> {
    ...
    return new BusinessProfile({ ...p, companyName: p.companyName ?? undefined, description: p.description ?? undefined, mission: p.mission ?? undefined, vision: p.vision ?? undefined });
  }
  ...
  async getProfileByUserId(userId: string): Promise<BusinessProfile | null> {
    ...
    return p ? new BusinessProfile({ ...p, companyName: p.companyName ?? undefined, description: p.description ?? undefined, mission: p.mission ?? undefined, vision: p.vision ?? undefined }) : null;
  }
